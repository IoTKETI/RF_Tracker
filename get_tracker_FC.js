...
function parseMavFromDrone(mavPacket) {
    try {
        ...
        if (msg_id === mavlink.MAVLINK_MSG_ID_GLOBAL_POSITION_INT) { // #33
            ...
            globalpositionint_msg.vx = Buffer.from(vx, 'hex').readInt16LE(0);
            globalpositionint_msg.vy = Buffer.from(vy, 'hex').readInt16LE(0);
            globalpositionint_msg.vz = Buffer.from(vz, 'hex').readInt16LE(0);
